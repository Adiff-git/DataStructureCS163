...
void AVLTreeVisualizer::drawAnimationMenu() {
    ...
    if (fastForwardButtonClicked) {
        // Bỏ điều kiện kiểm tra IDLE và currentPath.empty()
        // if (currentState != IDLE && !currentPath.empty()) {
            ...
            if (currentOperation == "insert" && !insertPaths.empty()) {
                ...
                if (currentState == SHOWING_RESULT && !tree.search(pendingInsertValue)) {
                ...
                if (currentState == SHOWING_RESULT && tree.search(operationValue)) {
    ...
    switch (currentState) {
        ...
        case SHOWING_RESULT:
            ...
            if (resultTimer >= duration) {
                // Không đặt về IDLE, giữ SHOWING_RESULT để tua lại được
                // currentState = IDLE;
                // currentPath.clear();
                pathIndex = 0;
                rotationIndex = 0;
